...
for L = lengths
    for cType = collisionTypes
        for v = 1:length(versions)
            ...
            acLx1_unfilt = diff(vcLx1)/0.05;
            acLy1_unfilt = diff(vcLy1)/0.05;
            acLx2_unfilt = diff(vcLx2)/0.05;
            acLy2_unfilt = diff(vcLy2)/0.05;
            acLx3_unfilt = diff(vcLx3)/0.05;
            acLy3_unfilt = diff(vcLy3)/0.05;
            acGx1_unfilt = diff(vcGx1)/0.05;
            acGy1_unfilt = diff(vcGy1)/0.05;
            acGx2_unfilt = diff(vcGx2)/0.05;
            acGy2_unfilt = diff(vcGy2)/0.05;
            acGx3_unfilt = diff(vcGx3)/0.05;
            acGy3_unfilt = diff(vcGy3)/0.05;
                 
            acLx1 = filter(b,a,acLx1_unfilt);
            acLy1 = filter(b,a,acLy1_unfilt);
            acLx2 = filter(b,a,acLx2_unfilt);
            acLy2 = filter(b,a,acLy2_unfilt);
            acLx3 = filter(b,a,acLx3_unfilt);
            acLy3 = filter(b,a,acLy3_unfilt);
            acGx1 = filter(b,a,acGx1_unfilt);
            acGy1 = filter(b,a,acGy1_unfilt);
            acGx2 = filter(b,a,acGx2_unfilt);
            acGy2 = filter(b,a,acGy2_unfilt);
            acGx3 = filter(b,a,acGx3_unfilt);
            acGy3 = filter(b,a,acGy3_unfilt);
            ...
            for i=1:num_points-2
                ...
                % Rotation matrix for the heading angles
                R1 = [cos(heading_angle1), -sin(heading_angle1);
                      sin(heading_angle1),  cos(heading_angle1)];
               
                
                R3 = [cos(heading_angle3), -sin(heading_angle3);
                      sin(heading_angle3),  cos(heading_angle3)];
                
                
                % Calculate transformed speeds
                [vc1Lx3, vc1Ly3] = transformSpeeds(psi1(i), vcLx3(i), vcLy3(i), psi3(i));
            
                % Calculate transformed accelerations
                [ac1Lx3, ac1Ly3] = transformAccelerations(psi1(i), acLx3(i), acLy3(i), psi3(i));
                ...
                % Calculate relative velocity
                a_rel_x31(i) = ac1Lx3 - acLx1(i);
                a_rel_y31(i) = ac1Ly3 - acLy1(i);
                ...
                eqnX = roots([a_rel_x31(i), 2*v_rel_x31(i), -2*abs(s_long31(i))]);
                TTCx =  eqnX(min(find(eqnX>0)));
            
                if isreal(TTCx) && isempty(TTCx)~=1
            
                  s_lat_f = abs(s_lat31(i))-v_rel_y31(i)*TTCx - 0.5*a_rel_y31(i)*TTCx^2;
                ...
                eqnY = roots([a_rel_y31(i), 2*v_rel_y31(i), -2*abs(s_lat31(i))]);
                TTCy =  eqnY(min(find(eqnY>0)));
            
                if isreal(TTCy) && isempty(TTCy)~=1
            
                   s_long_f= abs(s_long31(i))-v_rel_x31(i)*TTCy - 0.5*a_rel_x31(i)*TTCy^2;
                ...
                if(abs(s_lat_f) < w && v_rel_x31(i)>0 && abs(s_long31(i))>l1) && TTCx < 5
                    TTC_long1(i)= TTCx;
                else
                    TTC_long1(i)= 5;
                end    
                
                if(abs(s_long_f) <= l1 && v_rel_y31(i)>0 && abs(s_lat31(i))>w) && TTCy < 5
                    TTC_lat1(i)= TTCy;
                else
                    TTC_lat1(i)= 5;
                end  
                ...
                while (time<=tau1(i))
                    ...
                    X1(k) = X1(k-1) + vcGx1(i) * dt + 0.5 * acGx1(i) * dt.^2;
                    Y1(k) = Y1(k-1) + vcGy1(i) * dt + 0.5 * acGy1(i) * dt.^2;
                    X3(k) = X3(k-1) + vcGx3(i) * dt + 0.5 * acGx3(i) * dt.^2;
                    Y3(k) = Y3(k-1) + vcGy3(i) * dt + 0.5 * acGy3(i) * dt.^2;
                    
                    deltaX= X1(k-1)-X1(k);
            
                    A = abs(tan((psi_tr0 - psi_tc)/2));
                    B = deltaX/(l2*cos(psi_tc));
...
%%
      ...
      % Store the results
       ...
       fprintf("Processed: %s | y_target_1s = %.3f | y_target_2s = %.3f\n", fname, y_target_1s, y_target_1_3s, y_target_2s);
...
writetable(results, 'ytarget_results_acc.xlsx');
fprintf('\nAll results saved to ytarget_results_acc.xlsx\n');

%% Boxplot of y_target for each trailer length and collision type

% Read results from Excel
data = readtable('ytarget_results_acc.xlsx');
